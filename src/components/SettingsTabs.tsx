    ...
    <div>
          ...
          {tabs.map(({ key, labelKey, Icon }) => {
            ...
            return (
              <button
                ...
                className={[
                  'flex items-center gap-2 px-4 py-3 text-sm font-medium border-b-2 transition-colors duration-150 whitespace-nowrap outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 rounded-t-lg',
                  isActive
      ...
      <div role="tabpanel" className="animate-fade-in">
        {activeTab === 'subscription' && (
          <Card title={t('subscription')} className="max-w-2xl">
            <SubscriptionSection />
          </Card>
        )}
        {activeTab === 'moxie' && (
          <Card title={t('moxie')} className="max-w-2xl">
            <MoxieConnectionForm hasSubscription={hasSubscription} />
          </Card>
        )}
        {activeTab === 'billing' && (
          <Card title={t('billing')} className="max-w-2xl">
            <BillingProviderForm hasSubscription={hasSubscription} />
          </Card>
        )}
        {activeTab === 'currency' && (
          <Card title={t('currency')} className="max-w-2xl">
            <CurrencyForm hasSubscription={hasSubscription} />
          </Card>
        )}
        {activeTab === 'schedule' && (
          <Card title={t('schedule')} className="max-w-2xl">
